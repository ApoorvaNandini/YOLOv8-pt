...
def transform_2d_labels(min_x, min_y, max_x, max_y):
    center_x = (max_x + min_x) / (2*1600.0)
    center_y = (max_y + min_y) / (2*900.0)
    width = (max_x - min_x)/1600.0
    height = (max_y - min_y)/900.0
...
def get_2d_boxes(sample_data_token: str, visibilities: List[str]) -> List[OrderedDict]:
    ...
    for ann_rec in ann_recs:
        ...
        center_x, center_y, width, height = transform_2d_labels(min_x, min_y, max_x, max_y)


        # Generate dictionary record to be included in the .json file.
        repro_rec = generate_record(ann_rec, center_x, center_y, width, height, sample_data_token, sd_rec['filename'])
...
def main(args):
    ...
    # ---------------------
    df = pd.DataFrame(reprojections)
    print(df.columns.tolist())
    df["map_name"] = df.apply(lambda row: get_location_from_sample_token(row["sample_data_token"], nusc), axis=1)
    df = df[["category_name", "bbox_corners", "filename", "map_name"]]
    ...
    df.to_csv(os.path.join(args.dataroot, args.version, args.filename) ,index=False)
    # ---------------------

    print("Saved the 2D re-projections under {}".format(os.path.join(args.dataroot, args.version, args.filename)))

def get_location_from_sample_token(sample_data_token: str, nusc) -> str:
    ...
    map_name = nusc.get("log", log_token)["location"]
    return map_name

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Export 2D annotations from reprojections to a .json file.',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--dataroot', type=str, default='/home/asaridena/yolo-v8/v1.0-mini', help="Path where nuScenes is saved.")
    parser.add_argument('--version', type=str, default='v1.0-mini', help='Dataset version.')
    parser.add_argument('--filename', type=str, default='image_annotations.csv', help='Output filename.')
    ...
    parser.add_argument('--image_limit', type=int, default=10, help='Number of images to process or -1 to process all.')
...
"""
python convert_annotations.py --dataroot=/home/asaridena/yolo-v8/v1.0-mini --version=v1.0-mini --image_limit=-1 --filename=image_annotations.csv
"""
